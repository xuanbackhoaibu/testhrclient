...
const ACCESS_MODULES: AccessModuleDefinition[] = [
  {
    ...
    label: "Nhân sự",
    description:
      "Hồ sơ, điều chuyển, hợp đồng, onboarding, offboarding và phân công.",
    matches: (permission) =>
      [
        "hr.dashboard.",
        "hr.employee.",
  ...
  },
  {
    ...
    matches: (permission) =>
      ...
      permission.startsWith("hr.leave_balance."),
  },
  {
...
export function summarizeEffectiveAccess(
  ...
  if (unmatched.length > 0) {
    summaries.push({
      ...
      description:
        "Các quyền hiệu lực không thuộc nhóm nghiệp vụ HRM tiêu chuẩn.",
...
export function describeAccessScopes(scopes: readonly ScopeLike[]): string[] {
  ...
  const labels = scopes.map((scope) => {
    ...
    if (type === "UNIT")
      return count ? `${count} đơn vị được phân` : "Đơn vị được phân";
    if (type === "DEPARTMENT") {
