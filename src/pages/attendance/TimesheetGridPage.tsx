...
function cellDescription(day: TimesheetGridDay | undefined): string {
  ...
  return [
    day.holidayName,
    day.firstPunch && day.lastPunch ? `${day.firstPunch}–${day.lastPunch}` : null,
...
const TimesheetDataRow = memo(function TimesheetDataRow({
  ...
  const { row, daysByNumber, lateMinutes, earlyLeaveMinutes, explanationCount } = item;
  ...
  return (
    <Table.Tr>
      ...
      <Table.Td style={fixedStyle(52, 118)}>
        <Text size="sm" fw={600}>{row.employeeCode}</Text>
      </Table.Td>
      <Table.Td style={fixedStyle(170, 220)}>
        <Text size="sm" fw={600}>{row.fullName}</Text>
      </Table.Td>
      <Table.Td style={fixedStyle(390, 185)}>
        {row.jobTitle ? (
          <Text size="sm">{row.jobTitle}</Text>
        ) : (
          <Badge size="sm" color="orange" variant="light">Chưa gán chức vụ</Badge>
        )}
      </Table.Td>
      {dayMetas.map((meta) => {
        ...
        return (
          ...
          >
            <Text fw={label ? 700 : undefined} size="sm" c={label.includes('KL') ? 'red.9' : undefined}>
              {label || (day?.needsExplanation ? '?' : '·')}
            </Text>
          </Table.Td>
        );
      })}
      <Table.Td style={{ textAlign: 'center' }}><Text fw={700}>{row.summary.totalPaidDays}</Text></Table.Td>
      <Table.Td style={{ textAlign: 'center' }}>{row.summary.totalLeaveDays || '—'}</Table.Td>
      <Table.Td style={{ textAlign: 'center' }}>{lateMinutes || '—'}</Table.Td>
      <Table.Td style={{ textAlign: 'center' }}>{earlyLeaveMinutes || '—'}</Table.Td>
      <Table.Td style={{ textAlign: 'center' }}>
        {explanationCount ? <Badge size="sm" color="red" variant="light">{explanationCount}</Badge> : '—'}
      </Table.Td>
    </Table.Tr>
  );
});
...
export function TimesheetGridPage() {
  ...
  const dayMetas = useMemo(
    () => Array.from(
      { length: gridQuery.data?.daysInMonth ?? 31 },
  ...
  const preparedRows = useMemo<PreparedTimesheetRow[]>(
    () => rows.map((row) => ({
      ...
      lateMinutes: row.days.reduce((sum, day) => sum + day.lateMinutes, 0),
      earlyLeaveMinutes: row.days.reduce((sum, day) => sum + day.earlyLeaveMinutes, 0),
      explanationCount: row.days.filter((day) => day.needsExplanation).length,
    })),
    [rows],
  );
  const groupedRows = useMemo(() => {
    ...
    [...preparedRows]
      .sort((left, right) =>
        departmentGroupLabel(left.row).localeCompare(departmentGroupLabel(right.row), 'vi') ||
  ...
  const openCell = useCallback((row: TimesheetGridRow, day: TimesheetGridDay) => {
  ...
  async function handleSaveCell() {
    ...
    try {
      await adjustDay.mutateAsync({
        id: editing.day.id,
        payload: {
          segments: editSymbol ? [{ symbol: editSymbol, portion: editPortion }] : [],
      ...
      notifications.show({
        ...
        message: 'Thay đổi được lưu lịch sử và không bị cập nhật tự động ghi đè.',
      });
      setEditing(null);
    } catch {
      notifications.show({
    ...
    <>
      <PageHeader
        ...
        actions={canEdit || canExport ? (
          <Group gap="xs">
            {canExport ? (
              <Button variant="default" leftSection={<IconDownload size={18} />} loading={isExporting} onClick={() => void handleExport()}>
                Xuất Excel
              </Button>
            ) : null}
            {canEdit ? (
              <Button leftSection={<IconRefresh size={18} />} loading={recompute.isPending} onClick={() => void handleRecompute()}>
                Cập nhật bảng công
              </Button>
            ) : null}
          </Group>
      ...
      <Stack gap="md">
        <Alert icon={<IconInfoCircle size={18} />} color="blue" variant="light">
          Giờ hành chính <b>08:00–17:30</b>; chỉ check-in <b>sau 08:15</b> mới tính đi muộn. Thứ Bảy làm buổi sáng <b>08:00–12:00</b>. Ô “Chưa gán chức vụ” là hồ sơ chưa có phân công/chức danh hiệu lực.
        </Alert>

        <Paper withBorder radius="md" p="sm" shadow="xs">
          <Group justify="space-between" align="end" wrap="wrap">
            <Group align="end" gap="sm">
              <Select label="Kỳ công" w={142} data={monthOptions} value={String(month)} onChange={(value) => setMonth(Number(value ?? 1))} />
              <Select label="Năm" w={112} data={yearOptions} value={String(year)} onChange={(value) => setYear(Number(value ?? now.getFullYear()))} />
              <Stack gap={4}>
                <Text size="sm" fw={500}>Phạm vi xem & xuất</Text>
                <Button variant="default" leftSection={<IconFilter size={16} />} onClick={openScopeModal}>
                  {scopeLabel}
                </Button>
              </Stack>
            </Group>
            <Group gap="xs" pb={4}>
              <Badge variant="light" color="violet">{scopeLabel}</Badge>
              <Text size="sm" c="dimmed">{rows.length} nhân viên · {groupedRows.length} nhóm</Text>
            </Group>
          </Group>
        </Paper>
        ...
        {gridQuery.isLoading ? (
          ...
          <Alert color="gray" variant="light" title="Chưa có nhân sự trong phạm vi đang chọn">
            Kiểm tra lại công ty, phòng ban hoặc phân công hiệu lực của nhân sự rồi cập nhật lại bảng công.
          </Alert>
        ) : (
          <ScrollArea type="auto" offsetScrollbars>
            <Table withTableBorder highlightOnHover stickyHeader stickyHeaderOffset={0} style={{ minWidth: 575 + dayMetas.length * 52 + 410 }}>
              <Table.Thead>
                <Table.Tr>
                  {fixedColumns.map((column) => (
                    <Table.Th key={column.key} rowSpan={2} style={{ ...fixedStyle(column.left, column.width, true), textAlign: column.key === 'number' ? 'center' : 'left', verticalAlign: 'middle' }}>
                      {column.label}
                    </Table.Th>
                  ))}
                  {dayMetas.map((meta) => (
                    <Table.Th key={meta.day} style={{ minWidth: 52, textAlign: 'center', background: meta.isSunday ? '#ffe7a6' : '#e6f2df' }}>
                      {String(meta.day).padStart(2, '0')}
                    </Table.Th>
                  ))}
                  {summaryHeaders.map((label) => (
                    <Table.Th key={label} rowSpan={2} style={{ minWidth: 82, textAlign: 'center', verticalAlign: 'middle', background: '#e6f2df' }}>
                      {label}
                    </Table.Th>
                  ))}
                </Table.Tr>
              ...
              <Table.Tbody>
                {groupedRows.map((group, groupIndex) => (
                  <Fragment key={group.label}>
                    <Table.Tr>
                      <Table.Td colSpan={fixedColumns.length + dayMetas.length + summaryHeaders.length} style={{ background: '#d9d2e9', fontWeight: 700 }}>
                        {groupIndex + 1}. {group.label} <Text component="span" size="sm" c="dimmed">({group.rows.length} nhân viên)</Text>
                      </Table.Td>
                    </Table.Tr>
      ...
      <Modal opened={scopeModalOpened} onClose={() => setScopeModalOpened(false)} title="Phạm vi xem và xuất bảng công" size="lg" centered>
        <Stack gap="md">
          <Alert color="violet" variant="light">
            Không chọn mục nào nghĩa là xem và xuất <b>toàn công ty</b>. Có thể chọn nhiều đơn vị và phòng ban; kết quả được gộp trong một bảng công, phân nhóm rõ theo đơn vị/phòng ban.
          </Alert>
          <Group align="flex-start" grow>
            <Stack gap="xs">
              <Group justify="space-between">
                <Text fw={600}>Công ty / đơn vị</Text>
                <Button variant="subtle" size="compact-sm" onClick={() => setDraftUnitIds([])}>Bỏ chọn</Button>
              </Group>
              <ScrollArea h={250} type="auto">
                <Stack gap={8} pr="sm">
                  {unitOptions.map((option) => (
                    <Checkbox key={option.value} label={option.label} checked={draftUnitIds.includes(option.value)} onChange={() => toggleDraftSelection(option.value, draftUnitIds, setDraftUnitIds)} />
                  ))}
                </Stack>
              </ScrollArea>
            </Stack>
            <Stack gap="xs">
              <Group justify="space-between">
                ...
                <Button variant="subtle" size="compact-sm" onClick={() => setDraftDepartmentIds([])}>Bỏ chọn</Button>
              </Group>
              <ScrollArea h={250} type="auto">
                <Stack gap={8} pr="sm">
                  {departmentOptions.map((option) => (
                    <Checkbox key={option.value} label={option.label} checked={draftDepartmentIds.includes(option.value)} onChange={() => toggleDraftSelection(option.value, draftDepartmentIds, setDraftDepartmentIds)} />
                  ))}
                </Stack>
              </ScrollArea>
            </Stack>
          ...
          <Group justify="space-between" mt="xs">
            <Button variant="subtle" onClick={() => { setDraftUnitIds([]); setDraftDepartmentIds([]); }}>Xem toàn công ty</Button>
            <Group>
              <Button variant="default" onClick={() => setScopeModalOpened(false)}>Hủy</Button>
              <Button onClick={() => { setUnitIds(draftUnitIds); setDepartmentIds(draftDepartmentIds); setScopeModalOpened(false); }}>Áp dụng phạm vi</Button>
            </Group>
          </Group>
        </Stack>
      </Modal>

      <Modal opened={editing !== null} onClose={() => setEditing(null)} title={editing ? `Sửa ô: ${editing.row.fullName} — ngày ${editing.day.date}` : 'Sửa ô chấm công'} centered>
        <Stack gap="sm">
          {editing?.day.firstPunch || editing?.day.lastPunch ? (
            <Text size="sm" c="dimmed">Máy chấm công ghi nhận: {editing.day.firstPunch ?? '—'} → {editing.day.lastPunch ?? '—'}</Text>
          ) : (
            <Text size="sm" c="dimmed">Máy chấm công không có dữ liệu cho ngày này.</Text>
          )}
          <Select label="Ký hiệu" placeholder="Để trống = xóa ký hiệu khỏi ô" clearable searchable data={SYMBOL_OPTIONS.map((option) => ({ value: option.code, label: `${option.code} — ${option.name}` }))} value={editSymbol} onChange={(value) => { setEditSymbol(value); const option = SYMBOL_OPTIONS.find((item) => item.code === value); if (option) setEditPortion(option.defaultPortion); }} />
          <NumberInput label="Số công" description="Ca Thứ Bảy 08:00–12:00 tính 1 công theo cấu hình ca" min={0} max={2} step={0.5} decimalScale={1} value={editPortion} onChange={(value) => setEditPortion(Number(value))} disabled={!editSymbol} />
          <Textarea label="Lý do sửa" description="Bắt buộc — được lưu để đối chiếu khi có khiếu nại" withAsterisk minRows={2} value={editReason} onChange={(event) => setEditReason(event.currentTarget.value)} />
          <Alert color="orange" variant="light" icon={<IconTrash size={16} />}>Sau khi lưu, ô này sẽ không bị job cập nhật bảng công ghi đè.</Alert>
          <Group justify="flex-end" mt="md">
            <Button variant="default" onClick={() => setEditing(null)}>Hủy</Button>
            <Button loading={adjustDay.isPending} onClick={() => void handleSaveCell()}>Lưu thay đổi</Button>
          </Group>
        </Stack>
      </Modal>
    </>
