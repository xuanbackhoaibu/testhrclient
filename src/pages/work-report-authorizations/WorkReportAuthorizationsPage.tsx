...
export function WorkReportAuthorizationsPage() {
  ...
  const [activeScopeTab, setActiveScopeTab] = useState<'departments' | 'units' | 'corporation'>('departments');
  const [corporateConfirmation, setCorporateConfirmation] = useState<MatrixRow | null>(null);
  ...
  const corporation = useQuery({ queryKey: ['work-report-options', 'corporation'], queryFn: getCorporation, retry: false, enabled: drawer || Boolean(corporateConfirmation) });
  ...
  const corporateGrant = useMutation({
    mutationFn: (row: MatrixRow) => {
      const currentCorporate = row.permissions.filter((permission) => permission.type === 'CORPORATE_REPORT');
      const hasCorporate = currentCorporate.some((permission) => permission.actions.includes('AGGREGATE'));
      if (!corporation.data) throw new Error('Chưa tải được phạm vi Tổng công ty.');
      return createBusinessGrant({
        employeeId: row.employeeId,
        permissions: hasCorporate ? [] : [{ type: 'CORPORATE_REPORT', scopeId: corporation.data.scopeId, actions: ['AGGREGATE'] }],
        removedPermissions: hasCorporate ? currentCorporate : [],
      });
    },
    onSuccess: () => {
      notifications.show({ color: 'green', message: 'Đã cập nhật quyền Tổng công ty.' });
      setCorporateConfirmation(null);
      refresh();
    },
    onError: (error) => notifications.show({ color: 'red', message: error instanceof Error ? error.message : 'Không thể cập nhật quyền Tổng công ty.' }),
  });
  ...
  const openDrawer = (row: MatrixRow | null, batch = false, tab: 'departments' | 'units' | 'corporation' = 'departments') => {
    setBatchMode(batch); setActiveScopeTab(tab); setSubject(row); setSelectedEmployeeId(row?.employeeId ?? null); hydrateActivePermissions(batch ? null : row); setRemovedPermissions([]); setPreviewed(false); setStep(row && !batch ? 1 : 0); setDrawer(true);
  };
  ...
  const columns: DataTableColumn<MatrixRow>[] = [
    ...
    { key: 'department', header: 'Quyền cấp phòng', minWidth: 210, render: (row) => <ScopeCompact scopes={viewModels.get(row.employeeId)?.departmentPermissions ?? []} canManage={can(MANAGE)} addLabel="Thêm quyền xem báo cáo theo phòng ban" onEdit={() => openDrawer(row, false, 'departments')} /> },
    { key: 'unit', header: 'Quyền cấp đơn vị', minWidth: 210, render: (row) => <ScopeCompact scopes={viewModels.get(row.employeeId)?.unitPermissions ?? []} canManage={can(MANAGE)} addLabel="Thêm quyền xem báo cáo theo đơn vị" onEdit={() => openDrawer(row, false, 'units')} /> },
    { key: 'corporation', header: 'Quyền Tổng công ty', minWidth: 170, align: 'center', render: (row) => {
      const hasCorporate = Boolean(viewModels.get(row.employeeId)?.corporationPermission?.actions.includes('AGGREGATE'));
      const label = hasCorporate ? 'Có quyền xem báo cáo toàn Tổng công ty' : 'Chưa có quyền toàn Tổng công ty';
      return <Tooltip label={can(MANAGE) ? label : 'Bạn không có quyền thay đổi phân quyền này'} withArrow><Switch aria-label={label} checked={hasCorporate} disabled={!can(MANAGE) || corporateGrant.isPending} onChange={() => setCorporateConfirmation(row)} /></Tooltip>;
    } },
    ...
    { key: 'actions', header: 'Thao tác', minWidth: 56, render: (row) => <Tooltip label="Xem chi tiết" withArrow><ActionIcon aria-label={`Xem chi tiết ${row.fullName}`} variant="subtle" onClick={() => setDetail(row)}><IconEye size={16} /></ActionIcon></Tooltip> },
  ];
  ...
  const renderScopeActions = (checked: BusinessAction[], onChange: (action: BusinessAction) => void, label: string) => <SegmentedControl aria-label={`Quyền báo cáo: ${label}`} color={checked.includes('SUBMIT') ? 'green' : 'blue'} value={checked.includes('SUBMIT') ? 'READ_SUBMIT' : 'READ'} styles={{ indicator: { backgroundColor: checked.includes('SUBMIT') ? 'var(--mantine-color-green-filled)' : 'var(--mantine-color-blue-filled)' }, label: { fontWeight: 600, '&[data-active]': { color: 'white' } } }} data={[{ value: 'READ', label: 'Chỉ đọc' }, { value: 'READ_SUBMIT', label: 'Đọc và gửi' }]} onChange={(value) => {
  ...
  return <Stack gap="lg">
    ...
    <Modal opened={drawer} onClose={() => setDrawer(false)} title={<Stack gap={2}><Text fw={700}>{batchMode ? 'Cấp quyền báo cáo hàng loạt' : 'Cập nhật quyền báo cáo công việc'}</Text>{!batchMode && subject && <Text size="sm" c="dimmed">{subject.fullName} · {subject.employeeCode} · {subject.departmentName ?? 'Chưa xác định phòng ban'} · {subject.unitName ?? 'Chưa xác định đơn vị'}</Text>}</Stack>} size="xl" centered>
    ...
    <Modal opened={Boolean(detail)} onClose={() => setDetail(null)} title="Chi tiết quyền báo cáo công việc" size="xl" centered>
      {detail && <PermissionDetail view={viewModels.get(detail.employeeId) ?? workReportAuthorizationToEmployeePermissionViewModel(detail)} />}
    </Modal>
    <Modal opened={Boolean(corporateConfirmation)} onClose={() => setCorporateConfirmation(null)} title={corporateConfirmation?.permissions.some((permission) => permission.type === 'CORPORATE_REPORT' && permission.actions.includes('AGGREGATE')) ? 'Thu hồi quyền Tổng công ty?' : 'Xác nhận cấp quyền Tổng công ty'} centered size="lg">
      {corporateConfirmation && (() => {
        const hasCorporate = corporateConfirmation.permissions.some((permission) => permission.type === 'CORPORATE_REPORT' && permission.actions.includes('AGGREGATE'));
        return <Stack gap="md"><Alert color={hasCorporate ? 'orange' : 'yellow'}>{hasCorporate ? `Sau khi thu hồi, ${corporateConfirmation.fullName} sẽ không còn quyền xem báo cáo trong phạm vi toàn Tổng công ty. Các quyền cấp phòng và cấp đơn vị không bị ảnh hưởng.` : `Bạn đang cấp cho ${corporateConfirmation.fullName} quyền truy cập báo cáo công việc trong phạm vi toàn Tổng công ty. Đây là quyền có phạm vi lớn hơn quyền phòng ban và đơn vị.`}</Alert><Paper withBorder p="sm"><Text fw={600}>{corporateConfirmation.fullName}</Text><Text size="sm" c="dimmed">{corporateConfirmation.employeeCode} · {corporateConfirmation.unitName ?? 'Chưa xác định đơn vị'}</Text><Text size="sm" mt={6}>Phạm vi sau khi cập nhật: {hasCorporate ? 'không có quyền Tổng công ty' : 'xem báo cáo toàn Tổng công ty'}</Text></Paper><Group justify="flex-end"><Button variant="default" onClick={() => setCorporateConfirmation(null)}>{hasCorporate ? 'Giữ nguyên' : 'Hủy'}</Button><Button color={hasCorporate ? 'orange' : 'yellow'} onClick={() => corporateGrant.mutate(corporateConfirmation)} loading={corporateGrant.isPending}>{hasCorporate ? 'Xác nhận thu hồi' : 'Xác nhận cấp quyền'}</Button></Group></Stack>;
      })()}
    </Modal>
  </Stack>;
}

function ScopeCompact({ scopes, canManage, addLabel, onEdit }: { scopes: PermissionPresentation[]; canManage: boolean; addLabel: string; onEdit: () => void }) {
  const shown = scopes.slice(0, 2);
  return <Group gap={5} wrap="nowrap" align="center">
    {shown.map((scope) => <Tooltip key={`${scope.type}:${scope.scopeId}`} label={<Stack gap={2}><Text size="xs" fw={600}>{scope.name}{scope.code ? ` · ${scope.code}` : ''}</Text>{scope.ownerName && <Text size="xs">{scope.ownerName}{scope.ownerCode ? ` · ${scope.ownerCode}` : ''}</Text>}<Text size="xs">{scope.accessLabel} · {scope.statusLabel}</Text></Stack>} withArrow multiline w={260}><Badge variant="light" size="sm" style={{ maxWidth: 130, cursor: 'pointer', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }} onClick={onEdit}>{scope.name}</Badge></Tooltip>)}
    {scopes.length > 2 && <Tooltip label="Xem và cập nhật toàn bộ phạm vi" withArrow><Badge variant="outline" size="sm" style={{ cursor: 'pointer', whiteSpace: 'nowrap' }} onClick={onEdit}>+{scopes.length - 2} phạm vi</Badge></Tooltip>}
    {canManage && <Tooltip label={addLabel} withArrow><ActionIcon variant="light" size="sm" aria-label={addLabel} onClick={onEdit}><IconPlus size={15} /></ActionIcon></Tooltip>}
    {!scopes.length && !canManage && <Tooltip label="Bạn không có quyền thay đổi phân quyền này" withArrow><Text size="sm" c="dimmed">—</Text></Tooltip>}
  </Group>;
}

function PermissionDetail({ view }: { view: EmployeePermissionViewModel }) {
...
function Preview({ scopes, units, corporateActions, corporationName }: { scopes: Record<string, DepartmentScopeDraft>; units: Record<string, UnitScopeDraft>; corporateActions: BusinessAction[]; corporationName?: string }) {
  const isSubmit = (actions: BusinessAction[]) => actions.includes('SUBMIT');
  const label = (actions: BusinessAction[]) => isSubmit(actions) ? 'Đọc và gửi' : 'Chỉ đọc';
  const color = (actions: BusinessAction[]) => isSubmit(actions) ? 'green' : 'blue';
  const diff = (actions: BusinessAction[]) => <Stack gap={5}><Group gap="xs"><Text size="sm">Quyền sau áp dụng</Text><Badge color={color(actions)} variant="filled">{label(actions)}</Badge></Group><Text size="xs" c="dimmed">{isSubmit(actions) ? 'Được xem và gửi báo cáo tổng hợp lên cấp nhận báo cáo.' : 'Được xem báo cáo trong phạm vi này.'}</Text></Stack>;
  return <Stack gap="sm"><Alert color="blue" title="Tóm tắt thay đổi">{Object.keys(scopes).length + Object.keys(units).length + (corporateActions.length ? 1 : 0)} phạm vi sẽ được tạo mới hoặc cập nhật. Màu xanh dương là Chỉ đọc; màu xanh lá là Đọc và gửi.</Alert><Divider label="Cấp phòng" labelPosition="left" />{Object.values(scopes).map((scope) => <Paper key={scope.departmentId} withBorder p="sm" bg={isSubmit(scope.actions) ? 'green.0' : 'blue.0'}><Stack gap={5}><Group justify="space-between"><Text fw={600}>{scope.departmentName}</Text><Badge color={color(scope.actions)}>{label(scope.actions)}</Badge></Group><Text size="sm" c="dimmed">Đơn vị nhận báo cáo: {scope.unitName} · {scope.unitCode}</Text>{diff(scope.actions)}</Stack></Paper>)}<Divider label="Cấp đơn vị" labelPosition="left" />{Object.values(units).map((scope) => <Paper key={scope.unitId} withBorder p="sm" bg={isSubmit(scope.actions) ? 'green.0' : 'blue.0'}><Stack gap={5}><Group justify="space-between"><Text fw={600}>{scope.unitName} · {scope.unitCode}</Text><Badge color={color(scope.actions)}>{label(scope.actions)}</Badge></Group>{diff(scope.actions)}</Stack></Paper>)}{corporateActions.includes('AGGREGATE') ? <><Divider label="Tổng công ty" labelPosition="left" /><Paper withBorder p="sm" bg="violet.0"><Group justify="space-between"><Text fw={600}>{corporationName ?? 'Toàn Tổng công ty'}</Text><Badge color="violet">Tổng hợp TCT</Badge></Group><Text size="sm">Quyền sau áp dụng: <b>Tổng hợp báo cáo công việc toàn Tổng công ty</b></Text></Paper></> : null}</Stack>;
}
